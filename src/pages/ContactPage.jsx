    ...
    <section className="bg-white pt-10 pb-0 overflow-hidden">
      <div className="container-xl">
        ...
        <div className="relative grid grid-cols-1 lg:grid-cols-5 items-end gap-0">
          ...
          <div
            className="relative z-10 bg-white lg:col-span-2 pb-10 pt-6 pr-8"
          >
            <p className="text-xs font-bold tracking-[0.25em] uppercase text-gray-400 mb-4">İletişim</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 border-t border-gray-150 mt-0"
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
      {contactItems.map((item) => (
        <div key={item.label} className="bg-white border border-gray-100 p-6 shadow-sm hover:shadow-md transition-shadow">
          <div className="w-12 h-12 bg-brand-orange/10 text-brand-orange flex items-center justify-center mb-4">
            {item.icon}
          </div>
          <h3 className="font-bold text-brand-black mb-2 text-sm uppercase tracking-wide">{item.label}</h3>
          {item.href ? (
            <a href={item.href} className="block text-sm font-semibold text-brand-black hover:text-brand-orange transition-colors mb-0.5">
              {item.value}
            </a>
          ) : (
            <p className="text-sm font-semibold text-brand-black mb-0.5">{item.value}</p>
          )}
...
function ContactForm() {
  ...
  const inputCls = "w-full bg-white text-brand-black placeholder-gray-400 px-4 py-3 text-sm rounded-lg outline-none focus:ring-2 focus:ring-brand-black transition"
  const labelCls = "block text-white/70 text-[10px] font-bold uppercase tracking-widest mb-1.5"

  if (submitted) {
    return (
      <div className="text-center py-10">
        <svg className="w-14 h-14 text-white mx-auto mb-4" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <h3 className="text-xl font-extrabold text-white uppercase mb-2">Teşekkürler!</h3>
        <p className="text-white/80 text-sm mb-6">En kısa sürede size dönüş yapacağız.</p>
        <button
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="name" required value={formData.name} onChange={handleChange}
            placeholder="örn. Ahmet" className={inputCls} />
        </div>
        <div>
          <label className={labelCls}>Soyad</label>
          <input type="text" name="name" value={formData.name} onChange={handleChange}
            placeholder="örn. Yılmaz" className={inputCls} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="email" name="email" required value={formData.email} onChange={handleChange}
            placeholder="örn. ahmet@email.com" className={inputCls} />
        </div>
        <div>
          <label className={labelCls}>Telefon Numarası</label>
          <input type="tel" name="phone" value={formData.phone} onChange={handleChange}
            placeholder="örn. 0212 345 67 89" className={inputCls} />
        </div>
      </div>
    ...
    <div className="bg-brand-orange py-4">
      <div className="container-xl flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-3 text-white">
          ...
          <span className="font-bold text-sm">Acil Elektrik Arızası mı? 7/24 Hizmetinizdeyiz!</span>
        </div>
        <a
